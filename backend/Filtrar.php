...
if ($result->num_rows === 0) { // No hay resultados
...
while ($row = $result->fetch_assoc()) { // Mostrar resultados
    ...
    echo '<button name="evento">Ver Detalles</button>';
    echo '</div>';
    echo '</div>';
}
...
document.querySelectorAll('button[name="evento"]').forEach(button => {
    button.addEventListener('click', () => {
        // Aquí puedes redirigir a la página de detalles del evento
        href="../Frontend/evento.html?id=' . $row['ID_Evento'] . '"
    });
});
